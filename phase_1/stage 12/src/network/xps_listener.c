...
void listener_connection_handler(void *ptr)
{
    ...
    while (1)
    {
        ...
        xps_connection_t *client = xps_connection_create(listener->core, conn_sock_fd); // for connection create source and sink
        if (client == NULL)
        {
        ...
        {
            logger(LOG_INFO, "xps_listener_connection_handler()", "Creating upstream connection to 127.0.0.1:3000");
        ...
        else if (listener->port == 8002)
        {
            ...
            if (file == NULL)
            {
                logger(LOG_ERROR, "listener_connection_handler()", "xps_file_create() failed");
                xps_connection_destroy(client);
                continue;
            }
        ...
        else if (listener->port == 8003)
        {
            ...
            if (file == NULL)
            {
                logger(LOG_INFO, "listener_connection_handler()", "rejected file outside public directory");
                xps_connection_destroy(client);
                continue;
            }
            xps_pipe_create(listener->core, DEFAULT_PIPE_BUFF_THRESH, file->source, client->sink);
