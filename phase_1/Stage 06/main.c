...
int main()
{
    ...
    // Init lists -> listeners and connnections are stored as global vectors
    vec_init(&listeners);
    ...
    {
        /* create listener instance and add to listeners vector */
        xps_listener_create(epoll_fd, "0.0.0.0", port);
        logger(LOG_INFO, "main()", "Server listening on port %u", port);
    }

    /* run the event loop to monitor epoll events */
