...
// checks if the file is inside the public directory and has read permissions for others,
//  then opens the file using fopen() to create and initialize a new xps_file_s instance.
xps_file_t *xps_file_create(xps_core_t *core, const char *file_path, int *error)
{
    ...
    *error = E_FAIL;
    ...
    logger(LOG_DEBUG, "file_create()", "Requested file full path: %s", resolved_path);
    ...
    //  compares the first public_len characters of both paths.
    {
